  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === CBTT[chainId] ? 'BTT' : '',
        }),
      );
    },
    [dispatch],
...
export function useGelatoLimitOrderDetail(order: Order) {
  ...
  const inputCurrency = order.inputToken === NATIVE && chainId ? 'BTT' : order.inputToken;
  const outputCurrency = order.outputToken === NATIVE && chainId ? 'BTT' : order.outputToken;
