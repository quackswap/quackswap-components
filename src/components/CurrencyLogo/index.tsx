...
export default function CurrencyLogo({
  ...
  const srcs: string[] = useMemo(() => {
    if (
      ...
      currency === CBTT[ChainId.WAGMI] ||
      currency === CBTT[ChainId.COSTON]
    )
      return [];
  ...
  if (deepEqual(currency, CBTT[ChainId.BITTORRENT_MAINNET])) {
    return <AvaxLogo size={`${size}px`} />;
  } else if (deepEqual(currency, CBTT[ChainId.WAGMI])) {
    return <WgmLogo size={`${size}px`} />;
  } else if (deepEqual(currency, CBTT[ChainId.COSTON])) {
