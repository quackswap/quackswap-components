...
import { CBTT, JSBI, Token, TokenAmount, Trade } from '@@hotcrosscom/quackswap-sdk';
...
const LimitOrder: React.FC<Props> = ({ swapType, setSwapType, isLimitOrderVisible }) => {
  ...
  const getInputCurrency = () => {
    if (gelatoInputCurrency && gelatoInputCurrency?.symbol === CBTT[chainId].symbol) {
      return CBTT[chainId];
    } else if (inputTokenInfo && inputTokenInfo?.symbol === CBTT[chainId].symbol) {
      return CBTT[chainId];
  ...
  const getOutputCurrency = () => {
    if (gelatoOutputCurrency && gelatoOutputCurrency?.symbol === CBTT[chainId].symbol) {
      return CBTT[chainId];
    } else if (outputTokenInfo && outputTokenInfo?.symbol === CBTT[chainId].symbol) {
      return CBTT[chainId];
  ...
  const onCurrencySelect = useCallback(
    (currency) => {
      ...
      if (currency?.symbol === CBTT[chainId].symbol) {
